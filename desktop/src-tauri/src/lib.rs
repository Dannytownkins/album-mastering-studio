...
struct NativePlaybackSession {
    ...
    character_settings: Arc<RwLock<dsp::CharacterSettings>>,
...
fn start_native_ab_loop_playback(
    ...
    let character_settings = Arc::new(RwLock::new(dsp::CharacterSettings::default()));
    ...
    let thread_character_settings = character_settings.clone();
...
#[tauri::command]
fn update_character_chain(
    state: State<'_, NativePlaybackState>,
    settings: dsp::CharacterSettings,
) -> Result<NativePlaybackStatus, String> {
    let mut guard = state
        .session
        .lock()
    ...
    };
    {
        let mut snapshot = session
            .character_settings
            .write()
            .map_err(|_| "Native playback character settings lock poisoned.".to_string())?;
        *snapshot = settings;
    }
    Ok(native_playback_status_from_session(session))
}
...
fn render_rust_character(
    ...
    if clip.channels != 2 {
        return Err(format!(
            "Rust character Phase 3 expects stereo PCM16 WAV input; got {} channel(s).",
            clip.channels
        ));
    }
    if clip.samples.len() % 2 != 0 {
        return Err("Rust character Phase 3 received an incomplete stereo frame.".to_string());
    }

    dsp::process_character(&mut clip.samples, &settings);
    write_pcm16_wav(&output, &clip.samples, clip.channels, clip.sample_rate)?;
        ...
        .build_output_stream(
            ...
            move |data: &mut [T], _| {
                ...
                let current_mbc_settings = mbc_settings
                    ...
                    .map(|settings| *settings)
                    .unwrap_or_default();
                let current_character_settings = character_settings
                    .read()
                    .map(|settings| *settings)
                    .unwrap_or_default();
                ...
                for frame in data.chunks_mut(channels) {
                    ...
                    if !paused && channels >= 2 {
                        ...
                        let mut character_frame = [left, right];
                        dsp::process_character(&mut character_frame, &current_character_settings);
                        frame_values[0] = character_frame[0];
                        frame_values[1] = character_frame[1];
                    }
                    for (channel, sample) in frame.iter_mut().enumerate() {
