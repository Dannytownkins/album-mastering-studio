...
Operating state: resumed as of 2026-05-13. The latest full release trace at `e619318` has passed, and a later approval-scope hardening pass has targeted packaged verification. A fresh full release-readiness trace is still required from the app-code commit that includes that hardening before making a new full release-ready claim. The active goal remains open until the remaining listening, preview-scope, and native-dialog blockers are resolved or explicitly accepted.
...
| Release package | Latest full release-readiness trace at `e619318` rebuilt the sidecar, release EXE, MSI, and NSIS bundles and passed release, real-song, true headless native output probe, Native A/B UI/audio, installer, and diff gates. A later approval-scope hardening pass rebuilt the release app and passed targeted packaged smokes, but still needs a fresh full release trace. | Covered with listening/dialog caveats |
